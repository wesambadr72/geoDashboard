...
export default function GiniChart({ country,className }: { country: Country ,className?: string }) {

// تحضير البيانات وترتيبها حسب السنة تصاعدياً
 const giniData = useMemo(() => {
  if (!country.economy?.gini_coefficient) return [];
  return Object.entries(country.economy.gini_coefficient)
    .map(([year, value]) => ({
      year: year,
      value: Number(value)
    }))
    ...
    return (
    <Card dir="rtl" className={`w-full bg-card border border-border shadow-md rounded-2xl overflow-hidden font-tajawal transition-all duration-300 hover:shadow-lg ${className ?? ''}`}> 
